...
# ===== CONFIG =====
ROOT_DIR="$(pwd)"
PROJECT_PATH="AIPromptGenerator/Community.PowerToys.Run.Plugin.AIPromptGenerator/Community.PowerToys.Run.Plugin.AIPromptGenerator.csproj"
PLUGIN_NAME="AIPromptGenerator"
PUBLISH_DIR="AIPromptGenerator/Publish"

# ===== CLEAN UP =====
rm -rf "$PUBLISH_DIR"
rm -rf "AIPromptGenerator/Community.PowerToys.Run.Plugin.AIPromptGenerator/bin"
rm -rf "AIPromptGenerator/Community.PowerToys.Run.Plugin.AIPromptGenerator/obj"
rm -f "${ROOT_DIR}/${PLUGIN_NAME}-"*.zip

# ===== GET VERSION =====
VERSION=$(grep '"Version"' AIPromptGenerator/Community.PowerToys.Run.Plugin.AIPromptGenerator/plugin.json | sed 's/.*"Version": "\([^"]*\)".*/\1/')
...
package_build() {
    ...
    PUBLISH_PATH="./AIPromptGenerator/Community.PowerToys.Run.Plugin.AIPromptGenerator/bin/Release/net9.0-windows10.0.22621.0/$ARCH/publish"
    DEST="./AIPromptGenerator/Publish/$CLEAN_ARCH"
